...
export class ProfileComponent implements OnInit {
  ...
  loadUsers() {
    this.http.get<User[]>('http://68.183.146.53/usuarios/').subscribe(
      data => {
        this.users = data;
      },
      error => {
  ...
  deleteUser() {
    if (this.userIdToDelete !== null) {
      this.http.delete(`http://68.183.146.53/usuarios/${this.userIdToDelete}`).subscribe(
        () => {
  ...
  saveUser(user: User) {
    ...
    if (this.isEditMode) {
        ...
        this.http.put<User>(`http://68.183.146.53/usuarios/${user.id}`, userPayload).subscribe(
            response => {
        ...
        this.http.post<User>('http://68.183.146.53/usuarios/', userPayload).subscribe(
            response => {
