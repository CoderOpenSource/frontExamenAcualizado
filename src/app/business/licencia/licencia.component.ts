...
export class LicenciaComponent implements OnInit {
  ...
  loadLicencias() {
    this.http.get<Licencia[]>('http://68.183.146.53/licencias/').subscribe(
      data => {
        this.licencias = data;
      },
      error => {
  ...
  loadDocentes() {
    this.http.get<Docente[]>('http://68.183.146.53/docentes/').subscribe(
      data => {
        this.docentes = data;
      },
      error => {
  ...
  deleteLicencia() {
    if (this.licenciaIdToDelete !== null) {
      this.http.delete(`http://68.183.146.53/licencias/${this.licenciaIdToDelete}`).subscribe(
        () => {
  ...
  saveLicencia(licencia: Licencia) {
    ...
    this.http.post('http://68.183.146.53/licencias/createOrUpdate', formData).subscribe(
      () => {
